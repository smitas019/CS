...
public class DataStore {
	...
	private  SimpleDateFormat sdformat = new SimpleDateFormat("dd-M-yyyy hh:mm:ss");
	private List<Post> newsFeedsList = new ArrayList<Post>();
	public DataStore() {
		try {
	...
	public boolean updateNewsFeed() {
		...
		users.forEach(usr->{
			List<Post> userPostList=newsFeedsList.stream().sorted().filter(
					...
					).limit(20).map(post->post).collect(Collectors.toList());
	 ...
	 */
	private  void setNewsFeedsList() throws ParseException{
	 ...
	 */
	public  List<Post> getNewsFeedsList() {
